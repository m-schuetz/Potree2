...
fn main_vertex(vertex : VertexIn) -> VertexOut {
	...
	voxelIndex = voxelIndex + node.numVoxelsProcessed;
...
function getBindGroup(renderer, node, state){
	...
	if(!bindGroupCache.has(node)){
		...
		let vboPositions = renderer.getGpuBuffer(node.batch.buffers.position);
		let vboColors = renderer.getGpuBuffer(node.batch.buffers.color);
...
export function renderVoxelsLOD(root, drawstate){
...

	{
		...
		for(let i = 0; i < nodes.length; i++){
			...
			view.setUint32(32 * i + 12, node.numVoxelsProcessed, true);
			view.setUint32(32 * i + 16, node.numVoxels, true);
	...
	for(let node of nodes){
		...
		passEncoder.draw(36 * node.numVoxels, 1, 0, instanceIndex);
