  ...
  for (let pointAttribute of pointAttributes.attributes) {
    if (["POSITION_CARTESIAN", "position"].includes(pointAttribute.name)) {
      ...
      for (let j = 0; j < numPoints; j++) {
        ...
        if (pointOffset + 4 <= decoded.buffer.byteLength) {
          let mc_0 = view.getUint32(pointOffset + 4, true);
          // Other code that uses mc_0...
        } else {
          console.error(
            `Attempted to access data outside the buffer bounds. Byte offset: ${pointOffset}, Src index: ${src_index}, Num Points: ${numPoints}, Buffer length: ${decoded.buffer.byteLength}`
          );
        }
        let mc_0 = view.getUint32(pointOffset + 4, true);
