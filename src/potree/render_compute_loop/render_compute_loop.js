...
import {csDepth, depth_precompiled, group_size as group_size_depth} from "./csDepth.js";
import {csColor, color_precompiled, group_size as group_size_color} from "./csColor.js";
...
let bindGroupCache_depth = new Map();
...
function depthPass(renderer, octree, camera){
	...
	for(let node of nodes){
		...
		i++;
		if(i <= dbgRange.min || i >= dbgRange.max){
			continue;
		}

		let gpuBuffers = renderer.getGpuBuffers(node.geometry);

		let bindGroup = bindGroupCache_depth.get(node);
		...
		if(!bindGroup){
			...
			bindGroupCache_depth.set(node, bindGroup);
		}

		passEncoder.setBindGroup(0, bindGroup);

		// let groups = Math.ceil(node.geometry.numElements / group_size_depth);
		let groups = Math.ceil(node.geometry.numElements / (group_size_depth * 8));
...
export function renderComputeLoop(renderer, octree, camera){
