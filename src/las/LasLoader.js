...
export class LASLoader{
	...
	async *loadBatches(){
		...
		while(pointsLoaded < numPoints){
			...
			let colors = new Uint8Array(4 * currentBatchSize);
			...
			for(let i = 0; i < currentBatchSize; i++){
				...
				let r = view.getUint16(pointOffset + rgbOffset + 0);
				let g = view.getUint16(pointOffset + rgbOffset + 2);
				let b = view.getUint16(pointOffset + rgbOffset + 4);

				// 256 ** parseInt(Math.log(r) / Math.log(256))
